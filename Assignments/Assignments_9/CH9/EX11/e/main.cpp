...
class RandomSample
{
	...
	public:
		...
		{
			// theta = 2 * Pi * (n + d)
			double nd = angle / (2 * M_PI);
			double n = std::floor(nd);
			double d = nd - n;
			if (d > 0.5)
			{
				d = 1 - d;
		...
		SampleResult next()
		{
			...
			double n = (*nOfLambda_Ptr)(result.lambda);
			double alpha = std::asin(b);
			double beta = std::asin(b/n);
			result.theta1 = 4 * beta - 2 * alpha;
			result.theta2 = 6 * beta - 2 * alpha - M_PI;
			result.theta2 = correctAngle(result.theta2);
			return result;
		}
};
...
double (*simu())[nBinTheta]
{
	...
	for (size_t i = 0; i < nSamplePerProcess; i++)
	{
		...
		hist.accumulate(result.lambda, result.theta1);
		hist.accumulate(result.lambda, result.theta2);
	}
