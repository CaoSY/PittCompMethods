...
namespace WaveFunction
{
    ...
    Complex psi(double x);              // stationary state
    Complex psiT(double x, double t);   // time-dependent state
    ...
    Complex psiT(double x, double t)
    {
        return psi(x) * std::exp(Complex(0,-omega*t));
    }

    double realPsiT(double x, double t)
    {
        return psiT(x,t).real();
    }
    double imagPsiT(double x, double t)
    {
        return psiT(x,t).imag();
    }
    ...
    double estimateMaxDensity()
    {
        ...
        for (size_t i = 0; i < posOfBoundaries.size(); i++)
        {
            maxDensity = std::max(maxDensity, density(posOfBoundaries[i]));
        }
